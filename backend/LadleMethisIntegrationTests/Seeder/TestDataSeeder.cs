	...
	LadleMeThisContext context)
{
	...
	private async Task SeedUsersAsync()
	{
		...
		if (await userManager.FindByEmailAsync(testUser.Email) == null)
		{
			...
			{
				throw new Exception(
					$"Failed to create user {testUser.UserName}: {string.Join(", ", result.Errors)}");
			}
		}
	}
	...
	private async Task SeedCategoriesAsync()
	{
		...
		{
			var category = new Category { Name = "Morning Delights" };
	...
	private async Task SeedTagsAsync()
	{
		if (!await context.Tags.AnyAsync())
		{
			var tags = new[]
			{
				...
				new Tag { Name = "Test tag 4" },
				new Tag { Name = "Test tag 5" },
			};
